...
/**
 * 终幕文字层三段式时序（pin 释放点起算，单位为视口高的倍数）：
 * 定格后先滚过 FINALE_COPY_DELAY 屏文字保持隐藏，再花 FINALE_COPY_SPAN 屏
 * 在屏幕正中原地渐渐显影（位置一步不动），完全显示后再原地停留
 * FINALE_COPY_SETTLE 屏，才切钉层轨迹——与「快速带你前往」同速 1:1 上滑。
 * 显影与停留期间罗盘必须仍在一屏开外（屏幕上只有定格壁纸与文字），
 * 由 calibrateFinaleSpacer 校准 FinaleSpacer 衔接占位保证。
 */
const FINALE_COPY_DELAY = 0.12;
const FINALE_COPY_SPAN = 0.3;
/** 钉层间距：文字块底缘与罗盘标题顶缘保持半个视口高 */
...
function setupScenes(context: SetupContext) {
	...
	// 终幕文字层（可选，独立 fixed 层）：博客信息与快捷控件。与内容区的
	// 「快速带你前往」同层同速——由 renderExit 实测罗盘标题位置钉层驱动；
	// 未配置 finale 文案时节点不存在，用可选查询避免整层搭建失败
	const finaleCopyLayer = root.querySelector<HTMLElement>(
		"[data-finale-copy-layer]",
	);
	const finaleCopyInner = finaleCopyLayer?.querySelector<HTMLElement>(
		".home-blinds__finale-copy-inner",
	);
	// 钉层锚点：内容区首屏标题「快速带你前往」，文字块与其保持半屏间距同速上滑
	const compassTitle = document.querySelector<HTMLElement>(
		".home-compass__title",
	);
	// 终幕衔接占位（可选）：HomeBlinds 与内容区之间的空档，由
	// calibrateFinaleSpacer 实测校准高度，保证文字显影期间罗盘仍在
	// 一屏开外、轨迹接管点落在显影完成之后；未配置 finale 时不存在
	const finaleSpacer = root.parentElement?.querySelector<HTMLElement>(
		"[data-finale-spacer]",
	);
	...
	const renderExit = () => {
		...
		// 交接零跳变、间距恒为半屏
		if (finaleCopyLayer) {
			const entranceEnd = vh * (FINALE_COPY_DELAY + FINALE_COPY_SPAN);
			let copyAlpha: number;
			if (released <= vh * FINALE_COPY_DELAY) {
				// 第一段：定格后的缓冲，文字保持隐藏
				copyAlpha = 0;
			} else if (released < entranceEnd) {
				// 第二段：正中原地渐渐显影（easeOutCubic），位置不动
				const p =
					(released - vh * FINALE_COPY_DELAY) /
					(entranceEnd - vh * FINALE_COPY_DELAY);
				copyAlpha = 1 - (1 - p) ** 3;
			} else {
				// 第三段：完全显示，原地停留至轨迹接管点
				copyAlpha = 1;
			...
			if (compassTitle) {
				...
				y = Math.min(0, -(released - entranceEnd));
			}
			gsap.set(finaleCopyLayer, {
				autoAlpha: exitCovered ? 0 : copyAlpha,
				y,
			});
		}
	};
	...
	/**
	 * 终幕衔接距离校准：实测把「钉层轨迹过正中点」（文字与罗盘间距恰为
	 * 半屏、开始同速上滑的时刻，即 renderExit 里的 crossing）校准到
	 * 「显影完成 + FINALE_COPY_SETTLE 屏」处——做法是按差值补足
	 * FinaleSpacer 衔接占位的高度。
	 * 罗盘全程随文档流以斜率 -1 移动，compassTop + released 恒等于它在
	 * 「pin 释放时刻」的视口位置（C0），任意滚动位置都可实测；spacer 高度
	 * 一比一计入 C0，故增量一次解出。实测值自动吸收衔接带、布局间距等
	 * 一切占位差异，无需硬编码。
	 */
	const calibrateFinaleSpacer = () => {
		if (
			!pinTrigger ||
			!finaleCopyLayer ||
			!finaleCopyInner ||
			...
			return;
		const vh = Math.max(1, window.innerHeight);
		const halfBlock = finaleCopyInner.offsetHeight / 2;
		const crossingTarget =
			vh * (FINALE_COPY_DELAY + FINALE_COPY_SPAN + FINALE_COPY_SETTLE);
		const released = window.scrollY - pinTrigger.end;
		const c0 = compassTitle.getBoundingClientRect().top + released;
		// crossing = C0 + spacer - GAP*vh - halfBlock - vh/2
		// → 令 crossing = crossingTarget 解出 spacer 增量
		...
		const current = finaleSpacer.offsetHeight;
		const next = Math.max(0, current + delta);
		if (Math.abs(next - current) > 1) {
			finaleSpacer.style.height = `${next}px`;
		}
	};
	...
	function syncStage() {
		...
		if (phase === "done") {
			...
			// 已被内容背景盖住时跳过满屏可见态重置，防止先置 1 再被 renderExit
			// 盖回的间隙被 paint 成一帧背景大图（底部触底校正时偶发闪烁）
			if (!exitCovered) renderFinale(1);
			renderExit();
			return;
		}
	...
	// 字体加载会改变文字块高度（halfBlock），就绪后重新校准一次
	document.fonts?.ready.then(() => calibrateFinaleSpacer());
	...
	syncStage();
	// 初始校准一次衔接占位（refresh 事件触发后 handleRefresh 会再校准，
	// 这里兜底覆盖 refresh 前用户就滚到终幕的情形）
	calibrateFinaleSpacer();
...
function setupHeadline(context: SetupContext) {
	...
	// 中央圈内的主页文字组（取代原标题/祝福语），量宽与对齐都以它为准
	const emblem = headline.querySelector<HTMLElement>("[data-hero-emblem]");
	...
	// 迁移自 HomeHero 的身份信息层：中央圈文字组 + 右栏/对话框，
	// 跟随标题入场节奏，长条到位后整组淡入。未启用时节点不存在，全部走可选查询。
	const heroPanels = Array.from(
		root.querySelectorAll<HTMLElement>("[data-blinds-hero]"),
	);
	const heroWindow = root.querySelector<HTMLElement>("[data-hero-window]");
	...
	const measure = () => {
		...
		// 长条退开宽度按圈内文字组（无则退回整栈）的半宽反算
		const focal = emblem ?? stack;
		const gapHalf = focal.offsetWidth / 2 + window.innerWidth * 0.012;
		return {
			openScale:
				bandWidth > 0 ? clamp(1 - gapHalf / bandWidth, 0.06, 0.98) : 0.76,
			// 入场时文字组独占中线，落定后整栈居中（无第二层祝福语时两者一致）
			titleY: -focal.offsetHeight / 2,
	...
	const reset = () => {
		...
		heroWindow?.removeAttribute("data-blinds-hero-revealed");
		if (heroPanels.length > 0) gsap.set(heroPanels, { autoAlpha: 0, y: 18 });
	...
	const settle = () => {
		...
		// 落定即视为揭示完成：标题层与身份信息层都直接给到终态
		// （headline 的 autoAlpha 平时归 enter 时间线管，这里兜底防它停在隐藏态；
		//   退场仍由 setupReveal 的 scrub 时间线接管，滚过后会重新写回 0）
		gsap.set(headline, { yPercent: 0, autoAlpha: 1 });
		// 兜底通知：若首次进入即落定（如刷新时带滚动位置），回声入场同样要播
		root.dispatchEvent(new CustomEvent("home-blinds:headline-enter"));
		heroWindow?.setAttribute("data-blinds-hero-revealed", "");
		if (heroPanels.length > 0) gsap.set(heroPanels, { autoAlpha: 1, y: 0 });
	...
	const play = () => {
		if (played) return;
		played = true;
		// 通知回声文字层（站名 echo 效果）开始自己的入场编排
		root.dispatchEvent(new CustomEvent("home-blinds:headline-enter"));
		const { openScale, titleY, stackY, slide } = measure();
		const span = enterSpan;
		// 立即同步亮出标题层：不放进时间线，避免时间线建立前的任何时序问题
		// 让整层停在 reset 的隐藏态（圈内线条与文字全部不可见）
		gsap.set(headline, { yPercent: 0, autoAlpha: 1 });
		gsap.set(stack, { y: titleY });
		gsap.set(bands, { x: -slide, scaleX: 1 });
		...
		// 长条退开的后半程同步亮出身份信息层：先挂标记（供 CSS 过渡），
		// 再由时间线把整组文字推到终态，与标题落定节奏咬合
		if (heroPanels.length > 0 && heroWindow) {
			enter
				.add(
					() => heroWindow.setAttribute("data-blinds-hero-revealed", ""),
					span * 0.55,
				...
				.to(
					heroPanels,
					{ autoAlpha: 1, y: 0, duration: span * 0.42, ease: "power2.out" },
					span * 0.58,
				);
		}
	};
	...
	return () => {
		...
		if (heroPanels.length > 0) gsap.set(heroPanels, { autoAlpha: 0 });
	};
}

/**
 * 中央圈站名的「回声」效果（参考 reactbits.dev 的 echo-text）：
 * 前景保持一份清晰白字，身后叠若干同文案幽灵层 ——
 * 入场时自右侧散开再逐层收拢落位；指针靠近时各层按深度差速追随，形成拖影。
 * 幽灵层的模糊在创建时静态写死，动画只碰 transform / opacity，不逐帧重绘滤镜。
 */
function setupEchoText(context: SetupContext) {
	const { root, gsap, signal } = context;
	const host = root.querySelector<HTMLElement>("[data-echo-text]");
	if (!host) return () => {};

	const text = (host.textContent ?? "").trim();
	// 重建式初始化：媒体查询重启时同一 DOM 会二次进入，这里先清空再搭
	host.textContent = "";
	const front = document.createElement("span");
	front.className = "home-blinds-hero__name-front";
	front.textContent = text;
	host.appendChild(front);

	const ECHO_COUNT = 10;
	/** 越深的回声追得越慢（quickTo 时长按深度放大） */
	const LAG = 0.24;
	/** 入场散开与指针响应共用的最大位移（px） */
	const OFFSET = 36;
	/** 光标在该半径内会把文字拉向自己 */
	...
	const followers = Array.from({ length: ECHO_COUNT }, (_, index) => {
		const depth = (index + 1) / ECHO_COUNT;
		const layer = document.createElement("span");
		layer.className = "home-blinds-hero__name-echo";
		layer.setAttribute("aria-hidden", "true");
		layer.textContent = text;
		layer.style.filter = `blur(${(depth * MAX_BLUR).toFixed(2)}px)`;
		host.appendChild(layer);
		layers.push(layer);
		const duration = 0.16 + depth * LAG * 1.6;
		return {
			depth,
			toX: gsap.quickTo(layer, "x", { duration, ease: "power3.out" }),
			toY: gsap.quickTo(layer, "y", { duration, ease: "power3.out" }),
		};
	});

	// 指针拖影：光标靠近时整组字被拉向光标，深层回声位移更大、追得更慢，
	// 于是浅层已到、深层还在半路，出现「回声甩尾」
	const onPointerMove = (event: PointerEvent) => {
		const bounds = host.getBoundingClientRect();
		const dx = event.clientX - (bounds.left + bounds.width / 2);
		const dy = event.clientY - (bounds.top + bounds.height / 2);
		const distance = Math.hypot(dx, dy);
		const strength = Math.max(0, 1 - distance / CURSOR_RADIUS);
		if (strength <= 0) {
		...
		const nx = Math.max(-1, Math.min(1, dx / CURSOR_RADIUS));
		const ny = Math.max(-1, Math.min(1, dy / CURSOR_RADIUS));
		for (const follower of followers) {
			const reach = OFFSET * strength * (0.3 + follower.depth * 0.7);
			follower.toX(nx * reach);
			follower.toY(ny * reach * 0.6);
		}
	};
	root.addEventListener("pointermove", onPointerMove, { signal });

	// 入场：标题入场事件触发后，回声层从右侧散开位置收拢落位，越深越晚到、越透明
	const runEntrance = () => {
	...
	root.addEventListener("home-blinds:headline-enter", runEntrance, {
		once: true,
		signal,
	});

	return () => {
		gsap.killTweensOf([...layers, front]);
	};
}
...
async function initializeHomeBlinds(root: HTMLElement, generation: number) {
	...
	try {
		...
		cleanups.push(setupEchoText(context));
